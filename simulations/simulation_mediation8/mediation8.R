...
params = expand.grid(seed = 201:300,
...
onestep_estimator <- function(df){
    ...
    # use an irrelevant model as the misspecified model
    # original model: y|a, m, w ~ N(5 + 1.5a + 2m + 5m^2 + 0.3w, 4)
    # misspecified model: y|a, m, w ~ N(10 - 1.5a - 4m + 20m^2 - 0.4w, 16)
    df$mu_hat <- rnorm(n, 10 - 4 * df$m + 20 * df$m^2 - 0.4 * df$w, sd = 4)
...
# store the results
setwd('/projects/dbenkes/winn/mediation8')
