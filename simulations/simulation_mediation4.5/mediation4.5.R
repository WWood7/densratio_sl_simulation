...
# w1 ~ N(5, 4), truncated into [2, 8]
# w2 ~ N(0.5, 1), truncated into [0, 1]
# a|w1, w2 ~ Bernoulli(0.6 - 0.15I(w1>7.5) + 0.05I(w1<6) - 0.35I(w1<4) - 0.15I(w1>5)
#.                     + I(w2<0.5)w2^2 - 0.2I(w2>0.5)w2^2)
# m|w1, w2, a=1 ~ Beta(0.6w1 + 2w2, 0.7w1)
# m|w1, w2, a=0 ~ N(0.1w1 + w2, 1), truncated into [0, 1]
# y|a, m, w1, w2 ~ N(5 + 1.5a + 2m + 5m^2 + 0.3w1 + 5w2, 4)
setdata <- function(n){
    w1 <- rtnorm(n, mean = 5, sd = 2, lower = 2, upper = 8)
    w2 <- rtnorm(n, mean = 0.5, sd = 1, lower = 0, upper = 1)
    a <- rbinom(n, 1, 0.6 - 0.15*I(w1>7) + 0.05*I(w1<6) - 0.35*I(w1<4) - 0.15*I(w1>5) + 
                    I(w2<0.5) * w2^2 - I(w2>0.5) * 0.2*w2^2)
    m <- (a == 1) * rnorm(n,  0.5 * w1 + 4*w2, sd = 1 + 0.1*w1) + 
        (a == 0) * rnorm(n, 0.3 * w1 + 6 * w2, sd = 1 + w2)
    y <- rnorm(n, 5 + 1.5 * a + 2 * m + 0.5 * m^2 + 0.3 * w1 + 5*w2, sd = 2)
    df <- data.frame(w1 = w1, w2 = w2, a = a, m = m, y = y)
}

# calculate the true value of the target parameter
true_val_data <- setdata(10000000)
# E[E(Y|A=0, M, W)|A=1, W]]
# 5 + 2E(M|A=1, W) + 5E(M^2|A=1, W) + 0.3W1 + 5W2
moment1 <- 0.5 * true_val_data$w1 + 4 * true_val_data$w2
moment2 <- moment1^2 + (0.3 * true_val_data$w1)^2
intermediate <- 5 + 2 * moment1 + 0.5 * moment2 + 0.3 * true_val_data$w1 + 5 * true_val_data$w2
...
onestep_estimator <- function(df){
    ...
    sd_w1 <- sd(df$w1)
    sd_w2 <- sd(df$w2)
    df$m_sdd <- (df$m - mean(df$m)) / sd_m
    df$w1_sdd <- (df$w1 - mean(df$w1))/ sd_w1
    df$w2_sdd <- (df$w2 - mean(df$w2))/ sd_w2
    # define the tasks
    task1 <- sl3_Task$new(data = df, covariates = c('m_sdd', 'w1_sdd', 'w2_sdd'), outcome = 'indicator', folds = 5)
    ...
    df$ratio_sl <- sl_pres
    df$true_ratio <- dnorm(df$m,  0.5 * df$w1 + 4*df$w2, sd = 1 + 0.1 * df$w1) / 
        dnorm(df$m, 0.3 * df$w1 + 6 * df$w2, sd = 1 * df$w2)
...
# store the results
setwd('/projects/dbenkes/winn/mediation4')
