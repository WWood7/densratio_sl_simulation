...
lr4 <- Pipeline$new(Lrnr_densratio_classification$new(classifier = cl3, name = 'cl1'), 
                     Lrnr_densratio_classification$new(classifier = cl3, stage2 = TRUE, name = ''))
lr5 <- Pipeline$new(Lrnr_densratio_classification$new(classifier = cl4, name = 'cl2'), 
                    Lrnr_densratio_classification$new(classifier = cl4, stage2 = TRUE, name = ''))
lr6 <- Pipeline$new(Lrnr_densratio_classification$new(classifier = cl5, name = 'cl3'), 
...
test1 <- data.frame(
    w = 5,
    m = seq(0.01, 0.99, length.out = 100)
)
test1 <- get_tr(test1)
...
for (i in 1:3) {
    if (i == 3){
        ...
        p_tr <- ggplot(data = plotdf, aes(x = m)) +
            geom_line(aes(y = .data[[paste0("tr", i)]], color = "True Ratio")) +  # Map color inside aes for legend
            geom_line(aes(y = .data[[paste0("sl_predict", i)]], color = "Estimated Ratio")) +  # Map color inside aes for legend
            ...
            guides(color = guide_legend(override.aes = list(size = 2)))  # Adjust legend line size
    }
    else if (i == 1){
        p_tr <- ggplot(data = plotdf, aes(x = m)) +
            geom_line(aes(y = .data[[paste0("tr", i)]], color = "True Ratio")) +  # Map color inside aes for legend
