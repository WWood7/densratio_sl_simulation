...
# introduce functions
source('/Users/winnwu/projects/Benkeser_Lab/densratio_sl_simulation/local_exp/simulation_lmtp_/functions.R')

# generate test data (for computing hold-out risk)
set.seed(55)
test <- generate_data(10000)

train <- generate_data(400)
...
lr1 <- Lrnr_densratio_kernel$new(method = 'RuLSIF', kernel_num = 200, alpha = 0.8, name = 'lr1')
lr2 <- Lrnr_densratio_kernel$new(method = 'KLIEP', kernel_num = 200, name = 'lr2')
lr3 <- Lrnr_densratio_kernel$new(method = 'RuLSIF', kernel_num = 200, alpha = 0.5, name = 'lr3')
lr4 <- Lrnr_densratio_kernel$new(method = 'RuLSIF', kernel_num = 200, alpha = 0.2, name = 'lr4')
...
# calc MSE
true_value <- true_ratio1 * true_ratio2 * true_ratio3 *true_ratio4
sub_tr <- true_value[true_value <1]
sub_sl <- sl_pre[true_value <1]
sub_csl <- csl_pre[true_value <1]
sl_pre <- sl_pre1 * sl_pre2 * sl_pre3 * sl_pre4
csl_pre <- csl_pre1 * csl_pre2 * csl_pre3 * csl_pre4
sl_mse <- mean(abs(sl_pre - true_value)^(2))
csl_mse <- mean(abs(csl_pre - true_value)^(2))
