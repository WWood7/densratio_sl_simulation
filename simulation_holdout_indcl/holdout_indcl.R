...
lr4 <- Pipeline$new(Lrnr_densratio_classification$new(classifier = cl1, name = 'cl1'), 
                    Lrnr_densratio_classification$new(classifier = cl1, stage2 = TRUE, name = ''))
lr5 <- Pipeline$new(Lrnr_densratio_classification$new(classifier = cl2, name = 'cl2'), 
                    Lrnr_densratio_classification$new(classifier = cl2, stage2 = TRUE, name = ''))
lr6 <- Pipeline$new(Lrnr_densratio_classification$new(classifier = cl3, name = 'cl3'), 
                    Lrnr_densratio_classification$new(classifier = cl3, stage2 = TRUE, name = ''))
lr7 <- Pipeline$new(Lrnr_densratio_classification$new(classifier = cl4, name = 'cl4'), 
                    Lrnr_densratio_classification$new(classifier = cl4, stage2 = TRUE, name = ''))
lr8 <- Pipeline$new(Lrnr_densratio_classification$new(classifier = cl5, name = 'cl5'), 
                    Lrnr_densratio_classification$new(classifier = cl5, stage2 = TRUE, name = ''))

csl <- Pipeline$new(Lrnr_densratio_classification$new(classifier = csl, name = 'csl'), 
...
lr4_risk <- holdout_risk(sl_fit$learner_fits$`Pipeline(cl1->)`, test)
lr5_risk <- holdout_risk(sl_fit$learner_fits$`Pipeline(cl2->)`, test)
lr6_risk <- holdout_risk(sl_fit$learner_fits$`Pipeline(cl3->)`, test)
lr7_risk <- holdout_risk(sl_fit$learner_fits$`Pipeline(cl4->)`, test)
lr8_risk <- holdout_risk(sl_fit$learner_fits$`Pipeline(cl5->)`, test)
csl_risk <- holdout_risk(csl_fit, test)
result <- c(sl_risk, lr1_risk, lr2_risk, lr3_risk, lr4_risk, 
            lr5_risk, lr6_risk, lr7_risk, lr8_risk, csl_risk)

# store the results
setwd('/projects/dbenkes/winn/holdout_indcl')
