...
directory <- '/Users/winnwu/projects/Benkeser_Lab/simulation_results/mediation3.5'
ssize_list <- c(100, 300, 500, 800, 1000)
...
df <- data.frame(
    ...
    sample_size = rep(c('n=100', 'n=300', 'n=500', 'n=800', 'n=1000'), 2),
    group = rep(c('one-shot sl', 'separate csl'), each = 5)
)
df$sample_size <- factor(df$sample_size, levels = c('n=100', 'n=300', 'n=500', 'n=800', 'n=1000'))
...
ggplot(df, aes(x = sample_size, y = est_means, color = group, group = group)) +
    ...
    theme_minimal() +
    scale_x_discrete(limits = c('n=100', 'n=300', 'n=500', 'n=800', 'n=1000'), expand = c(0, 0))
